...
import { gmailProvider, hasGmailReadonlyScope } from '@/lib/jobs/email/providers/gmail';
...
async function finishRedirect(request: NextRequest, code: string) {
  ...
  const returnTo = safeAppPath(cookieStore.get(RETURN_COOKIE)?.value, '/jobs/settings');

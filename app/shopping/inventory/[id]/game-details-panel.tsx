...
function Fact({ label, value }: { label: string; value: string | number | null }) {
  return (
    <div>
      ...
      <dd className={value ? 'text-ink' : 'text-ink-faint'}>{value || 'Not listed'}</dd>
    </div>
  );
}
  ...
  return (
    <li className="flex gap-3 px-3 py-3">
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="text-[13px] text-ink-faint">{boxLine(candidate)}</p>
        {candidate.bggId != null && (
          <p className="font-mono text-[12px] text-ink-muted">BGG {candidate.bggId}</p>
        )}
  ...
  return (
    <section className="space-y-4 rounded-card border border-border bg-surface p-4">
      <div className="flex items-baseline justify-between gap-3">
        <h2 className="text-sm font-semibold text-ink">Game details</h2>
        {game.autoImported && (
      ...
      {game.needsConfirmation && (
        <div className="space-y-3 rounded-lg border border-accent-orange/30 bg-accent-orange/5 p-3">
          <div>
            <h3 className="text-sm font-semibold text-ink">Which box is on your shelf?</h3>
          ...
          <div className="flex gap-3 rounded-lg border border-border bg-surface p-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-[13px] text-ink-faint">{boxLine(game)}</p>
              {game.bggId != null && (
                <p className="font-mono text-[12px] text-ink-muted">BGG {game.bggId}</p>
              )}
          ...
          {game.candidates.length > 0 && (
            <div>
              <p className="mb-1 text-[13px] font-medium text-ink">Other editions we found</p>

  ...
  return (
    <div>
      <button
        ...
        aria-expanded={open}
        className="press flex w-full items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-left text-[13px] text-ink-muted hover:bg-surface hover:text-ink"
      >
      ...
      {open && (
        <div className="mt-1 space-y-1 rounded-lg border border-border bg-surface p-2">
          <label className="block text-[11px] font-medium text-ink-faint" htmlFor="attr-field">
            Property
          </label>
          <select
            id="attr-field"
            ...
            className="h-8 w-full rounded-md border border-border bg-canvas px-2 text-[13px] text-ink focus:border-brand focus:outline-none"
          >
              ...
              {facet.values.map((value) => {
                ...
                return (
                  <Link
                    key={value}
                    href={href}
                    className="block truncate rounded-md px-2 py-1 text-[13px] text-ink-muted hover:bg-canvas hover:text-ink"
                  >
                    {value}
                  </Link>
                );

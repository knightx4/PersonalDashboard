...
export function VaultStatusBanner({ connection }: { connection: VaultConnectionSummary }) {
  if (connection.status === 'needs_reauth') {
    return (
      <div className="mb-5 flex items-start gap-2.5 rounded-card border border-border bg-caution-tint px-4 py-3">
        <AlertTriangle className="mt-0.5 size-4 shrink-0 text-caution" strokeWidth={2} aria-hidden />
  ...
  if (!connection.backfillCompletedAt) {
    return (
      <div className="mb-5 flex items-start gap-2.5 rounded-card border border-border bg-accent-tint px-4 py-3">
        <Loader2 className="mt-0.5 size-4 shrink-0 text-accent" strokeWidth={2} aria-hidden />
        <p className="text-body text-ink">

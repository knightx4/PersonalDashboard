...
### `thoughts`
`user_id`, `body`, `created_at`, `updated_at`. Free writing about the search on the Thoughts tab: what the person wants from a job and how that has changed. No structured fields, by design. A change of view is a new dated entry rather than an edit, so the history of it survives. The Goals routine reads the newest entries when it plans or reviews a career goal and when it searches for job leads (migration 0026).

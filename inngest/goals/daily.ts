 ...
 *
 * Fires the goals routine once for the owner when a Claude step is ready,
 * with the run row written first and the steps named in its brief. It
 * starts nothing when the routine is not set on the deployment, when a
 * morning run already started in the last twenty hours, or when no Claude
 * step is ready, because each run spends the owner's routine allowance. A
...
export type GoalsDailyResult =
  | { skipped: string }
  | { started: true; runId: string; steps: number; held: number };
...
export async function runGoalsDaily(deps?: Partial<GoalsDailyDeps>): Promise<GoalsDailyResult> {
  ...
  if (ready.length === 0) return { skipped: 'no Claude steps are ready' };
  ...
  const result = await recordAndFire({
    ...
    itemId: null,
    routine,
    text: (runId) => dailyRunText({ userId, runId, steps }),
    fetch: deps?.fetch,
  });
  ...
  return { started: true, runId: result.runId, steps: steps.length, held: ready.length - steps.length };
}

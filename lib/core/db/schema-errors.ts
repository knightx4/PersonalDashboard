 ...
 *
 * Five schemas back this app -- `public`, `job_search`, `core`, `obsidian`
 * (the vault workspace) and `todo` -- but
  ...
  constructor(readonly schema: string) {
    super(
      `The "${schema}" schema is not exposed by the API, so nothing in it can be read or written. ` +
        ...
        'includes public, job_search, core, obsidian and todo. No migration or deploy is needed — the change ' +

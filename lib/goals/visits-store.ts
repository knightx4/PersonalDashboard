...
type VisitRow = { last_visit_at: string; away_from: string | null; back_on: string | null };
...
export async function recordVisit(
  ...
  const { data, error } = await client
    .from('visits')
    .select('last_visit_at, away_from, back_on')
    .eq('user_id', userId)
    .maybeSingle();
  ...
  const record = nextVisit(
    row ? { lastVisitAt: row.last_visit_at, awayFrom: row.away_from, backOn: row.back_on } : null,
    now,
    today,
  );

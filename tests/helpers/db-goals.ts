/**
 * Test database helpers for the goals schema.
 *
 * An eighth helper for an eighth schema, on the same database. Which one a test
...
export const sql = postgres(TEST_DATABASE_URL, {
  max: 4,
  onnotice: () => {},
  connection: { search_path: `${GOALS_SCHEMA}, public, extensions` },
});
...
export async function asUser<T>(
  ...
  return sql.begin(async (tx) => {
    ...
    await tx.unsafe(`set local search_path = ${GOALS_SCHEMA}, public, extensions`);
    await tx.unsafe(
...
/** Everything in goals hangs off auth.users. */

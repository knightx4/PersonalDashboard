/**
 * Test database helpers for the learn schema.
 *
...
export const sql = postgres(TEST_DATABASE_URL, {
  max: 4,
  onnotice: () => {},
  connection: { search_path: `${LEARN_SCHEMA}, public, extensions` },
});
...
export async function asUser<T>(
  ...
  return sql.begin(async (tx) => {
    ...
    await tx.unsafe(`set local search_path = ${LEARN_SCHEMA}, public, extensions`);
    await tx.unsafe(
...
/** Everything in learn hangs off auth.users. */

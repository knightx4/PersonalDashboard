...
const PUSH: PushedCommit = {
  ref: 'claude/overnight-card-633',
  sha: 'a4f0912',
...
describe('the Overnight card while a night is running', () => {
  ...
  it('falls back to the branch when GitHub would not name the commit', () => {
    const html = draw({ push: { ...PUSH, subject: null } });
    expect(html).toContain('claude/overnight-card-633');
  });

  it('says a refusal from GitHub in the sentence the key needs, in place', () => {
    const html = draw({
      push: null,
      pushError: 'GITHUB_READ_TOKEN is missing a permission (403). Give it access.',
    });
    expect(html).toContain('Could not read what was pushed.');
...
describe('the Overnight card before the browser clock arrives', () => {
  it('leaves every clock-derived figure out rather than drawing it from zero', async () => {
    ...
    const html = renderToStaticMarkup(
      <AtZero run={run()} canSend night={night()} push={PUSH} pushError={null} />,
    );
